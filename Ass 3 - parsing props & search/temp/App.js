...
class App extends React.Component {
  ...
  searchFeed = (searchTerm) => {
    ...
    {
      this.setState({
        filteredEvents: events.filter((event) => event.description.includes(searchTerm)),
      });
    }
    else
    {
      this.setState({
        filteredEvents: events.filter((event) => event.name.toLowerCase().includes(searchTerm.toLowerCase())),
      });
    }
  };
...
// const root = ReactDOM.createRoot(document.getElementById("root"));
// root.render(<Event name={testEvent.name} date={testEvent.date} description={testEvent.description} />);
